...
**The Leads page and the Root/Social dashboards call their third-party APIs live, on every page load — by deliberate choice, not oversight.** This used to go through a "5-minute background cron writes to our table, the page only ever reads that table" cache pattern (a `meta-ads-leads-sync.job.ts` and a `social-analytics-cache-refresh.job.ts`), but both cron jobs were removed: the always-fresh data was judged more valuable than avoiding the real, measured round trip (Meta Graph ~7-8s for Leads, Upload-Post analytics ~several seconds for the dashboards). `GET /api/meta-ads/leads` calls `LeadsService.syncFromMeta()` before reading `leads`; the Root/Social dashboard routes call `UploadPostService.getProfileAnalytics`/`getTotalImpressions` directly. Don't reintroduce a background sync job for either without being asked — if page-load latency becomes a real problem again, that's a decision for whoever's asking to make explicitly, not something to silently restore.
...
**Leads has no webhook — `GET /api/meta-ads/leads` calls Meta live on every request instead.** It calls `LeadsService.syncFromMeta()` (upserts on `meta_lead_id`, never deletes) before reading `leads`, so the page always shows current data at the cost of a real, measured 7-8s round trip on every load — a deliberate tradeoff (there used to be a 5-minute background cron doing this instead; it was removed on purpose, see the Background jobs section above). Because `syncFromMeta()` only upserts, any lead from an earlier sync stays in `leads` even if a later Graph API response omits it, so reading the table right after syncing already reflects both sources, not just the latest call. If the live sync itself fails, the route degrades to whatever's already in `leads` rather than failing the page. The "Sync now" button (`POST /api/meta-ads/leads/sync`, same underlying service) still exists for forcing another sync without leaving/reloading the page. No `META_APP_SECRET`/`META_WEBHOOK_VERIFY_TOKEN`/HMAC verification/dashboard subscription needed either way.

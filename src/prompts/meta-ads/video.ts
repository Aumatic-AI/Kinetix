...
BEFORE (every single Phase 1 and Phase 2 scene, no exceptions): the protagonist has significant, obvious, unmistakable hair loss — MOSTLY BALD. Hair must be visibly absent across most of the scalp, with only a small, sparse amount remaining at the very top/crown of the head. This is NOT "a receding hairline" or "thinning hair" — it is a clearly bald-appearing head with minimal hair left. Use this exact same balding description, in equivalent words, in every single Phase 1 and Phase 2 scene's prompt — never vary it, never show fuller hair in some earlier scenes than others, never soften it to "some hair loss."
AFTER (every single Phase 3 scene, no exceptions): the protagonist now has a full, thick, healthy head of hair — good density, good volume, natural coverage across the entire scalp. This must look OBVIOUSLY and UNMISTAKABLY different from the "before" balding look, not a modest improvement.
Never show the AFTER (full-hair) look in any Phase 1 or Phase 2 scene, and never show the BEFORE (mostly-bald) look in any Phase 3 scene — the change happens once, at the treatment point in the story, not gradually and not early.
...
WHERE a logo must NEVER appear: anywhere on the protagonist's (or any customer's) own body, clothing, bag, jewelry, or any other personal item — a patient or customer does not wear or carry the clinic's branding, and putting it there is exactly the kind of mistake to avoid. This also, always, includes any mobile phone, laptop, tablet, or computer/monitor screen shown in ANY scene — never depict the logo on a screen (as an app icon, a website, a photo, a video-call window, or any other on-screen graphic), and never on the physical body/case of the device either, regardless of whose device it is or whether it's personal or belongs to the business itself. A screen shown in a scene, if it displays anything at all, must show generic or blank content — never this business's logo or branding. If a scene shows a personal item or any screen at all, it must stay generic/unbranded, even if the same scene also shows real signage elsewhere in frame.

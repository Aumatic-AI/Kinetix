...
WHERE a logo may plausibly appear — the business's own physical building ONLY, and in at most one or two scenes across the WHOLE video, never scattered onto many different items: its exterior signage/entrance/storefront front, or a single wall-mounted sign inside its lobby/reception area — that's it. This is how it actually looks in the real world: a hospital or clinic's logo lives on the building itself, not on every surface inside it. Never on a staff uniform, name badge, folder, form, brochure, desk item, front-desk counter, or any other printed or personal material — those are exactly the over-branded placements to avoid; a logo repeated across many different objects reads as fake, not authentic. If a given scene isn't actually about the building/its exterior or entrance, don't add branding to it just because the reference image is available.

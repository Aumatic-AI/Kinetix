...
Before writing Act 1, silently work out: what specific real-world condition or dissatisfaction is this idea actually about${descriptor ? ` — this is for "${descriptor}", so ground your answer in what that service actually treats` : ""}? Then write 2-3 concrete, natural ways a real person would describe that exact condition (specific and sensory, never a vague "something is wrong"). Use that grounding for every Act 1 line below — adapt the pronoun to the character's gender.
...
STEP 1 — DIAGNOSE THE CONDITION AND ITS SENSITIVITY (if the script is about a physical/health transformation)
Read the full script${descriptor ? ` (this is for "${descriptor}")` : ""} and identify the specific real-world condition or dissatisfaction it's actually about — describe it in plain words, not a category label. Then decide its depiction tier:

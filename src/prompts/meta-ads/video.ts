...
export const AD_MODE_STRUCTURES: Record<string, string> = {
  ...
  The moment can be absolutely anything a real person's life actually touches — home, family, a relationship, a hobby, exercise, travel, a health checkup, a quiet moment completely alone, or, yes, sometimes work — never assume or default to any one of these just because it's common or was used last time. Read the idea itself and invent whichever specific moment it actually calls for; if the idea doesn't point to one clearly, pick a genuinely different kind of moment each time rather than settling on the same reliable choice repeatedly. Two different scripts for the same rough idea should very plausibly land on two different moments, not the same one twice.

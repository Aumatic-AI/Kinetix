...
HARD RULE — THE IDEA'S OWN CONTENT MUST SURVIVE INTO EVERY ACT, NO MATTER WHICH MODE YOU PICKED — THIS OUTRANKS EACH ACT'S OWN WORDING BELOW
Whichever mode you chose above, the act structure is a SHAPE to pour the idea into, not a replacement for it. Every specific detail, claim, phrase, or instruction the idea above actually contains must show up somewhere in the script you write — you may ADD supporting detail beyond what the idea says, but you may never DROP, water down, or silently swap out something it explicitly says just because the chosen mode's template beat doesn't obviously fit it. If the idea has its own distinctive angle, phrasing, image, or voice, that specific angle must keep surfacing in EVERY act, not just the hook — Act 2 and Act 3 are not a free pass to abandon the idea and fall back on generic business facts once the hook is done. When an act's own instruction below says something like "grounded in the business's own context," that means USE the real business facts to SUPPORT and illustrate the idea's own angle — it does NOT mean set the idea aside and describe the business/services/offers generically instead; if a business fact and the idea's own angle ever seem to pull in different directions for a given line, the idea wins and the fact either gets reframed to serve it or gets left out. A useful test before you write each act: could this exact line have been written without ever reading the idea, using only the business's generic info? If yes, it's drifted — rewrite it so it could only make sense for THIS specific idea. Before returning, silently check: does every distinct thing the idea said actually appear somewhere in the script, and does the idea's specific angle still recognizably run through Acts 2 and 3, not just Act 1? If either answer is no, revise before returning.

...
// ============================================================
// REPORT ANALYSIS — on-demand only (never automatic), one GPT call
// analyzes every currently-loaded ad at once rather than one call per
// ad. Ported from the legacy project's /api/meta/report-analysis route:
// same deterministic-score-first design (GPT explains, never sets, the
// score), same per-pattern suggestion rules, same anti-hallucination
// guardrails.
// ============================================================

export interface ReportAdInput {
  adId: string;
  adName: string;
  campaignName: string;
  status: string;
...
function classifyPattern(ad: ReportAdInput): "A" | "B" | "C" | "D" {
  if (ad.impressions === 0 && ad.spend === 0) return "A";
  if (ad.impressions > 0 && ad.clicks === 0) return "B";
  if (ad.clicks > 0 && ad.ctr < 0.3) return "C";
  return "D";
}

export function getReportAnalysisPrompt(ads: ReportAdInput[], summary: { totalSpend: number; totalImpressions: number; totalClicks: number; avgCtr: number; avgCpm: number }): string {
  const gptData = ads.map((a) => ({
    ad_id: a.adId,
    ad_name: a.adName,
    ...
    metrics: { spend: a.spend, impressions: a.impressions, clicks: a.clicks, ctr_pct: a.ctr, cpm: a.cpm },
  }));

  return `You are analyzing a Meta Ads account for a medical tourism / healthcare brand. Ads have already been scored by a deterministic formula (CTR-driven) — you explain the scores and write specific, actionable suggestions, you never change or second-guess them.

## ACCOUNT TOTALS
spend=$${summary.totalSpend.toFixed(2)} | impressions=${summary.totalImpressions.toLocaleString()} | clicks=${summary.totalClicks} | avg_ctr=${summary.avgCtr}% | avg_cpm=$${summary.avgCpm.toFixed(2)}

## ADS
${JSON.stringify(gptData, null, 2)}

## PATTERNS (already classified per ad above)
A — never served (spend=0, impressions=0): only suggest a delivery fix (budget/status). Never suggest creative changes — they're irrelevant until the ad runs.
B — impressions>0, 0 clicks: the hook is failing. Suggest a headline/body rewrite, only if has_creative_data is true.
C — some clicks but CTR<0.3%: audience or relevance problem. Suggest audience/targeting changes, not creative.
D — everything else: whatever the data suggests — could be scaling a winner or a smaller efficiency tweak.

## SUGGESTION QUALITY RULE
A suggestion field must be null unless you can write something concrete and specific to THIS ad's real data. Fewer good suggestions beats many generic ones. Never output a suggestion that just restates the current headline/body verbatim — it must be a genuine rewrite. Never invent procedures, prices, or claims not present in the ad's own headline/body.

## ANTI-HALLUCINATION
1. If headline=null AND body=null, output null for headline_suggestion/body_suggestion — do not guess from the ad's name.
2. Every number in your writing must come from the metrics object exactly as given.
3. Never output [bracket] placeholder text as a final suggestion.

## REQUIRED JSON — return ONLY this, no markdown fences, no commentary
{
  "ai_overview": "2-3 sentences using real numbers — total spend, avg CTR, the single biggest opportunity",
  "key_insights": ["insight referencing a real ad name and metric", "..."],
  "ad_notes": [
    {
      "ad_id": "<id>",
      "issue": "root cause in one sentence with real numbers",
      "headline_suggestion": "<specific rewrite or null>",
      "body_suggestion": "<specific rewrite or null>",
      "budget_suggestion": "<concrete $ action or null>",
      "targeting_suggestion": "<specific audience action or null>"
    }
  ],
  "overall_recommendation": "the single highest-ROI action, naming a specific ad and an exact next step"
}
"ad_notes" must have exactly ${ads.length} entries, one per ad above, in the same order.`;
}

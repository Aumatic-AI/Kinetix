...
export function Competitors() {
  ...
  return (
    <div className="space-y-8 pb-10">
      <div>
        <h2 className="text-2xl font-bold text-text">Market Intelligence</h2>
        <p className="text-muted text-sm mt-1">
          {generatedAt ? `Last synced ${new Date(generatedAt).toLocaleDateString()}` : "Updated automatically every week."}
        </p>
      </div>
      ...
      {!hasAnyData ? (
        ...
        <>
          ...
          <div className="relative overflow-hidden rounded-2xl border border-primary/15 bg-gradient-to-br from-primary-subtle/60 via-primary-subtle/20 to-transparent px-6 py-5">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2 min-w-0">
                ...
                {bestHookFormula && (
                  <p className="text-xs text-primary font-semibold">{bestHookFormula}</p>
                )}
              </div>
            </div>
          </div>

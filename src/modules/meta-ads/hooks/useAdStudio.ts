...
export function useStudioSession(sessionId: string | null) {
  return useQuery({
    ...
    queryFn: () => fetchJson<{ session: StudioSession; messages: StudioMessage[] }>(`/api/meta-ads/studio/sessions/${sessionId}`),
    enabled: !!sessionId,
...
export function useFinalizeStudioSession(sessionId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: () => fetchJson<{ creativeId: string }>(`/api/meta-ads/studio/sessions/${sessionId}/finalize`, { method: "POST" }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: studioKeys.session(sessionId) }),
  });
}
